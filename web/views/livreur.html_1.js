        ...
        function renderActive(list) {
            ...
            list.forEach(o => {
                ...
                if (h) {
                    chatSection = `
                        <div class="chat-history-box" id="chat-box-${o.id}">
                            <span style="opacity:0.4; font-size:10px;">${t('dernier_echange', 'Dernier échange')} (${count}/6) :</span><br>
                    ...
                    chatSection = `
                        <div class="chat-history-box" id="chat-box-${o.id}">
                            <span style="opacity:0.4; font-size:10px;">${t('discussion_entamee', 'Discussion entamée')} (${count}/6)</span>
                        </div>
                    `;
                } else {
                    chatSection = `
                ...
                d.innerHTML = `
                    ...
                    <div style="font-size:10px; font-weight:900; color:var(--accent); margin-bottom:8px; opacity:0.8; letter-spacing:1px;">${t('approche_client', 'APPROCHE CLIENT :')}</div>
                    ...
                    <div class="btn-group" style="margin-bottom:10px;">
                        <button class="btn-action btn-primary btn-full" onclick="updateStatus('${o.id}', 'delivered')">✅ ${t('marquer_livre', 'MARQUER LIVRÉ')}</button>
                        <button class="btn-action btn-secondary" onclick="openNav('${o.address}')">🗺 ${t('naviguer', 'NAVIGUER')}</button>
                        <button class="btn-action btn-secondary" onclick="toggleChatModule('${o.id}')">💬 ${t('chat', 'CHAT')} (${count}/6)</button>
                    </div>
                    
                    <div class="btn-group" style="grid-template-columns:1fr 1fr; gap:10px;">
                        <button class="btn-action btn-secondary" style="height:38px; font-size:11px; color:#ffaa00; border-color:rgba(255,170,0,0.2);" onclick="updateStatus('${o.id}', 'abandoned')">⚠️ ${t('remettre', 'REMETTRE')}</button>
                        <button class="btn-action btn-secondary" style="height:38px; font-size:11px; color:#ff4444; border-color:rgba(255,68,68,0.2);" onclick="updateStatus('${o.id}', 'cancelled')">🚩 ${t('annuler', 'ANNULER')}</button>
                    </div>

                    <div class="chat-module" id="chat-module-${o.id}" style="display:${prevState.isOpen ? 'block' : 'none'};">
                        <div style="font-size:10px; font-weight:900; opacity:0.5; margin-bottom:8px; letter-spacing:1px;">${t('messagerie_anonyme', 'MESSAGERIE ANONYME :')}</div>
                        ${chatSection}
                        <div class="chat-input-group">
                            <input type="text" class="chat-input" id="chat-input-${o.id}" value="${safeQuote(prevState.text)}" placeholder="${t('msg_to_client', 'Votre message au client...')}" data-i18n-placeholder="msg_to_client" onkeydown="if(event.key==='Enter') sendChatMessage('${o.id}')">
                            <button class="btn-send" onclick="sendChatMessage('${o.id}')">${t('envoyer', 'ENVOYER')}</button>
                        </div>
                        <div style="font-size:9px; opacity:0.3; margin-top:6px; text-align:center;">${t('le_client_verra', 'Le client verra ce message sans connaître votre numéro.')}</div>
                    </div>
                `;
                cont.appendChild(d);
        ...
        function renderChatTabList() {
            ...
            list.forEach(o => {
                ...
                if (h) {
                    ...
                    chatHistoryHtml = `
                        <div class="chat-history-box" id="tab-chat-box-${o.id}">
                            <span style="opacity:0.4; font-size:10px;">${t('aucun_message', 'Aucun message échangé')} (${count}/6)</span>
                        </div>
                    `;
                }
                ...
                d.innerHTML = `
                    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">
                        <div style="font-weight:900; font-size:14px;"><i class="ph-fill ph-receipt"></i> ${t('commande', 'Commande')} #${o.id.slice(-5).toUpperCase()}</div>
                    ...
                    <div class="chat-input-group" style="margin-top:12px; display:flex; gap:8px;">
                        <input type="text" class="chat-input" id="tab-chat-input-${o.id}" placeholder="${t('msg_to_client', 'Message au client...')}" onkeydown="if(event.key==='Enter') sendTabChatMessage('${o.id}')"
                        ...
                        <button class="btn-send" onclick="sendTabChatMessage('${o.id}')" 
                            style="background:var(--accent); border:none; border-radius:12px; color:#fff; padding:0 15px; font-size:12px; font-weight:800; cursor:pointer;">${t('envoyer', 'ENVOYER')}</button>
                    </div>
                `;
                cont.appendChild(d);
            });
        ...
        function renderAvailable(list) {
            ...
            list.forEach(o => {
                ...
                d.innerHTML = `
                    ...
                    <div class="order-address"><span>📍</span>${o.address}</div>
                    <button class="btn-action btn-primary btn-full" onclick="takeOrder('${o.id}')">📦 ${t('accepter_la_course', 'ACCEPTER LA COURSE')}</button>
                `;
                cont.appendChild(d);
            });
        }
        ...
        async function loadHistory() {
            ...
            try {
                ...
                if (list.length > 0) {
                    ...
                    sum.innerHTML = `<div style="font-size:11px; font-weight:900; opacity:0.5; color:#00ff88; letter-spacing:1px; margin-bottom:5px;">${t('total_encaisse', 'TOTAL ENCAISSÉ')}</div><div style="font-size:24px; font-weight:900; color:#00ff88;">${total.toFixed(2)}€</div>`;
                    cont.prepend(sum);
                }
            } catch (e) {
        ...
        async function updateStatus(id, status) {
            const msgs = {
                'delivered': t("confirmer_livraison", "Confirmer la livraison de cette commande ?"),
                'abandoned': t("confirmer_remettre", "Remettre cette livraison dans la file d'attente globale ?"),
                'cancelled': t("confirmer_annuler", "Annuler définitivement cette commande (incident/stock indisponible) ?")
            };
            tg.showConfirm(msgs[status] || t("confirmer_action", "Confirmer l'action ?"), async (ok) => {
                if (!ok) return;
                try {
